...
if "features" not in bronze_df.columns:
    spark.stop()
    raise ValueError(
        "Invalid Bronze payload: 'features' field is missing."
    )

feature_count = bronze_df.select(explode("features")).count()

if feature_count == 0:
    spark.stop()
    raise ValueError(
        "Bronze payload contains zero features. "
        "Pipeline stopped to prevent empty Silver/Gold publication."
    )

print("Bronze feature count:", feature_count)
# ============================================================
...
ingest_timestamp = ingest_datetime.strftime(
    "%Y-%m-%d %H:%M:%S"
)

ingest_run_id = ingest_datetime.strftime(
    "%Y%m%d_%H%M%S"
)
